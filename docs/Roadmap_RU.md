...
- добавить опциональный Modrinth CDN fallback по exact file hash
  Статус: выполнено в `1.0.96`; `enable_modrinth_cdn_fallback` теперь включён по умолчанию, клиент сначала запрашивает Modrinth version-file API по `sha1` и только потом падает обратно на серверный URL
...
- убрать self-update лаунчер для самого `modsync.jar`, чтобы сборки проходили ограничения площадок распространения
  Статус: выполнено в `1.0.93`; staged post-exit self-update удалён из distributed build
